...
def RetrieveInfoFromPMID(pubmed_ids, data_folder, email):
	...
	print("## Retrieving GEO datasets for the papers...(this may take long)", end='\n')
	...
	chunks = split(pubmed_ids, 1000)
	# Processing the epost and efetch process for each chunk
	print("\n## Retrieving PubDetails for the papers...", end='\n')
	for i,chunk in enumerate(chunks):
		PrintInline("	+-- {0} chunk (at least 1,000) of papers...".format(i+1))
		...
		try:
			...
			for record in records:
				...
				# updating dictionary
				papers_infos[pmid] = {"author":au, "journal":journal, "title":title, "abstract":abstract, "pub_date":pub_date, "mesh_headings":mesh_headings, "gse_codes":gse_codes, "platforms":platforms, "ftp_links":ftp_links, "analysis":""}
...
def SortMesh(type, mesh_terms, data_folder):
	if type == "level":
		...
		for index, mh in enumerate(mesh_terms):
			...
			except (urllib.error.HTTPError, urllib.URLError, RuntimeError, KeyError) as e:
				WaitForSucceed(e)
...
def PerformAnalyses(gse_folder, tFile, eFile, analyses, ngenes, cont):
	# defining dictionary of commands for the selected analyses
	analysis_command = {
		"pca": {"script_name":"PCA.R", "-e": eFile, "-t": tFile, "-c": "target", "-n": cont, "-d": gse_folder},
		"receptor_status": {"script_name":"mclust.R", "-e": eFile, "-t": tFile, "-c": "target", "-n": cont, "-d": gse_folder},
		"tumour_purity": {"script_name":"estimate.R", "-e": eFile, "-t": tFile, "-c": "target", "-n": cont, "-d": gse_folder},
		"molecular_classification": {"script_name":"pam50.R", "-e": eFile, "-t": tFile, "-c": "target", "-n": cont, "-d": gse_folder},
		"gene_expression": {"script_name":"gene_expr.R", "-e": eFile, "-t": tFile, "-c": "target", "-n": cont, "-d": gse_folder, "-g": ngenes}
	}
	...
	for an in analyses:
		...
		if an in analysis_command.keys():
			r_cmd = ['Rscript']
			# dinamically build the Rscript command
			r_cmd.append("scripts/DataAnalysis/{0}".format(analysis_command[an]["script_name"]))
			arguments = dict(list(analysis_command[an].items())[1:])
			...
			# Getting log of analysed files
			try:
				subprocess.check_call(r_cmd, stderr=subprocess.STDOUT)
...
# This function check if a specific folder is empty. If yes, it performs the removal.
def CheckEmptyFolder(folder):
	# Here we put a try because we need to be sure the folder exists
	fname = folder.split("/")[-1]
	try:
		if len(os.listdir(folder)) == 0:
			print(exec_1_warn % fname)
			os.rmdir(folder)
	except:
		print(exec_2_warn % fname)
