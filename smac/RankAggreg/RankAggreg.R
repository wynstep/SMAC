...
function(x, k, weights=NULL, method=c("CE", "GA"),
		...
		convIn=ifelse(method=="CE", 7, 30), importance=rep(1,nrow(x)),
            ...
            popSize=100, CP=.4, MP=.01, verbose=TRUE, output_dir=NULL, ...)
{
    ...
    if(method=="CE"){
		...
		if(t==1)
            ...
            if(verbose){
		Sys.sleep(0.1)
  		cat("\r", paste0("Iteration for entropy minimisation: ", t))
                #cat("\n", "Iteration", t, ": ",  c("Optimal value: ", min(f.y),
                #        "\n Optimal List:  ", y.l, "\n"))
        # create png file
        png(filename = paste0(output_dir,"/update_",t,".png"))
		...
		switchWith <- sample(comp.list, mutations, replace=TRUE)
 		...
 		fyRes <- rbind(fyRes,c(min(f.y), median(f.y)))
            ...
            if(verbose){
                #cat("\n", "Iteration", t, ": ",  c("Optimal value: ", min(f.y),
                #        "\n Optimal List:  ", y.l, "\n"))
              png(filename = paste0(graph_dir,"/update_",t,".png"))
