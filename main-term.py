...
parser.add_argument('-t','--queryTerm', help='Term for starting a PubMed search query', required=True)
...
anCode = [character for character in args.queryTerm if character.isalnum()]
anCode = "".join(anCode)
...
logFile = logFile.format(outRootFolder, anCode)
	## Initialise recoverFlag
recFlag = 0

##---------------------------------------##
##	Extracting list of PMIDs from Pubmed ##
##---------------------------------------##
	# We aim at recovering any previous analysis already present
pmidFile = pmidFile.format(outRootFolder, anCode)
	# Loading PMIDs in previous run
if (os.path.isfile(pmidFile)):
	with open(pmidFile, "r") as pmidData:
		for line in pmidData:
			pPMIDs.append(line.rstrip())
	# Downloading current PMID list and calculate difference with previous
	allPMIDs = RetrievePMIDs(args.queryTerm, args.PMIDsLimit, args.email, logFile)
	PMIDs = list(set(allPMIDs) - set(pPMIDs))
	## At this point, the recover flag can be 1 (new papers found) or 2 (no new papers)
	if (len(PMIDs) > 0):
		recFlag = 1
		KeepLog(logFile, "[MESSAGE] Recovery mode ON: New PMIDs: {0}\n".format(len(PMIDs)))
	else:
		recFlag = 2
		sys.exit("[MESSAGE] Recovery mode ON: New PMIDs: {0} -- Analysis will quit NOW".format(len(PMIDs)))
else: # No previous run has been found
	PMIDs = RetrievePMIDs(args.queryTerm, args.PMIDsLimit, args.email, logFile)
# Append/write the new PMIDs to the previous file
SaveIntoDF(PMIDs, pmidFile, "list")
...
##--------------------------------------------------------------##
##	Create 	PPI and functional enrichment 						##
##--------------------------------------------------------------##
...
try:
	del tmpPapersInfo, papersInfoFile
except:
	KeepLog(logFile, "[WARNING] No need to clear VAR\n")
gc.collect()
...
if (len(meshInfo) > 0):
	...
	for j, m in enumerate(meshGAboundanceTermsSelected["mesh"].tolist()):
		KeepLog(logFile, "Retrieve MeSH global aboundance for {0}/{1} -- {2}\n".format(j+1, len(meshGAboundanceTermsSelected["mesh"].tolist()), m))
		# We need to put a timeout to the function, otherwise (if blocked IP or connection, it will stay for forever)
		try:
