...
#===============================================================================
	...
	.libPaths("/home/immunolab-brain/R/x86_64-pc-linux-gnu-library/3.6"); # update libpath
...
#===============================================================================
	...
	if (length(gIndex) > 0) {
		...
		#===============================================================================
			...
			exprData.numeric.zscore.selected <- rbind(head(exprData.numeric.zscore[,-1], args$nGenes), tail(exprData.numeric.zscore[,-1], args$nGenes))
			...
			#===============================================================================
			geneConverted <- bitr(rownames(exprData.numeric.zscore.selected), fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
			rownames(geneConverted) <- geneConverted$SYMBOL
			exprData.numeric.zscore.selected.converted <- merge(geneConverted, exprData.numeric.zscore.selected, by="row.names")
			# Generate gene list of Top 50/Last 50 scaled genes
			geneList <- exprData.numeric.zscore.selected.converted$ENTREZID
	...
	# The analysis cannot continue because we'll never map to genes
	} else {
		cat("Sorry, the analysis cannot continue, no valid gene names found")
	}
